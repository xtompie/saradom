...
Notify lets elements on a page tell each other that something changed. `nup` (short for notify up) sends a signal up through the ancestors. `ndown` (notify down) sends it down through the descendants. `nall` (notify all) sends it to the whole document. A listener is an attribute that holds a callback, run when the signal reaches it. Three calls and an attribute are enough to wire a page's pieces together.
...
A third argument is a selector where the walk stops. `nup` runs listeners up to it but not past it. `ndown` will not descend into it. The boundary element itself is not run either way, so a listener belongs inside it, or the boundary goes one level up. It is only needed when a widget is nested inside another of the same kind, so each keeps its signals to itself.
...
`nall` runs every listener in the document, wherever it sits. It is for a sender and a receiver with no useful common ancestor: a button in the top bar that collapses the sidebar, a toast that any part of the page can raise.

```html
<div sidebar-toggle="() => this.flag('sidebar-off', !this.flag('sidebar-off'))">...</div>

<button onclick="this.nall('sidebar-toggle')">Collapse</button>
```
